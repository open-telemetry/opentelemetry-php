...
class Span implements API\Span
{
    ...
    public function end(API\Clock $moment = null): API\Span
    {
        if (!isset($this->end)) {
            $this->end = $moment ?? (new Clock())->moment();
        }

        return $this;
    }

    public function setStartTimestamp(Clock $moment): Span
    {
        $this->start = $moment;
    ...
    public function getStartTimestamp(): API\Clock
    {
        return $this->start;
    }

    public function getEndTimestamp(): API\Clock
    {
        return $this->end;
    }
    ...
    public function addEvent(string $name, ?API\Attributes $attributes = null, ?API\Clock $moment = null): API\Span
    {
        // todo: really throw if not recording?
        $this->throwIfNotRecording();

        $this->events->addEvent($name, $attributes, $moment);
