...
trace examples: FORCE
	...
	$(DC_RUN_PHP) php ./examples/traces/features/exporters/always_on_zipkin_example.php
	$(DC_RUN_PHP) php ./examples/traces/features/always_off_trace_example.php
	$(DC_RUN_PHP) php ./examples/traces/features/exporters/always_on_jaeger_example.php
        # The following examples do not use the DC_RUN_PHP global because they need environment variables.
	docker-compose run -e NEW_RELIC_ENDPOINT -e NEW_RELIC_INSERT_KEY --rm php php ./examples/traces/features/exporters/always_on_newrelic_example.php
	docker-compose run -e NEW_RELIC_ENDPOINT -e NEW_RELIC_INSERT_KEY --rm php php ./examples/traces/features/exporters/always_on_zipkin_to_newrelic_example.php
	docker-compose stop
collector:
	docker-compose -f docker-compose.collector.yaml up -d --remove-orphans
	docker-compose -f docker-compose.collector.yaml run -e OTEL_EXPORTER_OTLP_ENDPOINT=collector:4317 --rm php php ./examples/traces/features/exporters/always_on_otlp_grpc_example.php
