...
benchmark: ## Run phpbench
	$(DC_RUN_PHP) env XDEBUG_MODE=off tools/phpbench run --report=default
phpmetrics: ## Run php metrics
	$(DC_RUN_PHP) env XDEBUG_MODE=off tools/phpmetrics --config=./phpmetrics.json --junit=junit.xml
...
rector-write: ## Run rector
	$(DC_RUN_PHP) env XDEBUG_MODE=off tools/rector process src
rector: ## Run rector (dry-run)
	$(DC_RUN_PHP) env XDEBUG_MODE=off tools/rector process src --dry-run
deptrac: ## Run deptrac
